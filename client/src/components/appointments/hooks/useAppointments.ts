...
export function useAppointments(): UseAppointments {
  ...
  useEffect(() => {
    const nextMonthInfo = getNewMonthYear(monthYear, 1);
    queryClient.prefetchQuery(
      [queryKeys.appointments, nextMonthInfo.month, monthYear.year],
      () => getAppointments(nextMonthInfo.year, nextMonthInfo.month),
      commonOptions
    );
  }, [monthYear.month, monthYear.year, queryClient, commonOptions]);
  ...
  const { data: appointments = {} } = useQuery(
    ...
    {
      select: showAll ? undefined : selectFun,
      ...commonOptions,
      refetchOnMount: true,
      refetchOnReconnect: true,
      refetchOnWindowFocus: true,
      refetchInterval: 60000, // every min; poll
    }
  );
